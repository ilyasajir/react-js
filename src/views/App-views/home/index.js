...
const Home = ({getProducts,product})=> {
    ...
    return(
        <div className="w-full space-y-8">
            <div className="h-60 w-full rounded-2xl bg-white overflow-hidden relative">
                <img src="/assets/img/banner-2.jfif" alt="" className={'absolute top-1/2 left-1/2 transform -translate-x-2/4 -translate-y-2/4 w-full'}/>
            </div>
                ...
                <div className={'w-full grid grid-cols-12 gap-6'}>
                    {product.collections.loading ? <Skeleton active className={'col-span-12'}/>:product.collections.data.map((item,index)=> (
                        <div className="h-60 col-span-2 rounded-2xl relative transition duration-200 overflow-hidden" key={index}>
                            <Link to={`/product/${item.id}`} className="w-full z-10 h-auto bg-black bg-opacity-50 absolute bottom-0 left-0 px-4 py-2">
                                ...
                                <p className={'truncate text-lg p-0 m-0 text-yellow-200 font-bold '}>{"Rp. " +item.price}</p>
                            </Link>
